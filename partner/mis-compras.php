...
<html lang="es">
    ...
    <body class="nav-fixed">
                                        ...
                                        <tbody>
                                            <?php
                                              $sql = "SELECT
                                                      ...
                                                      WHERE
                                                          pedidos.idCliente = '$emailUsuario' AND pedidos.isActive = 1 ORDER BY pedidos.fechaPedido DESC";
                                              ...
                                              if (mysqli_num_rows($result) > 0)
                                              {
                                                  ...
                                                  while($row = mysqli_fetch_assoc($result))
                                                  {
                                                      ...
                                                      echo "<tr>";
                                                        ...
                                                        echo "<td class='fw-600'><a target='_blank' href='" . $dominio ."/". $row["idTienda"] . "'>" . $row["idTienda"] . "</a></td>";
                                                        ...
                                                        {
                                                            ?>
                                                            <div class='btn btn-success btn-sm rounded-pill mb-1'><i class='fas fa-check-circle fa-lg me-2'></i> Entregado</div>
                                                            <?php
                                                        ...
                                                        {
                                                            ?>
                                                            <div class="btn btn-danger p-2 rounded-pill">Pago Pendiente</div>
                                                            <?php
                                                        }
                                                        else
                                                        {
                                                        ...
                                                        <td class='text-center'>
                                                            <a class="btn btn-outline-blue rounded-pill fw-600" href="detalleCompra.php?id=<?php echo $row['idPedido']; ?>">
        ...
        <script type="text/javascript">
            ...
            const someCheckbox = document.getElementById('reqEnvio');
            const output = document.getElementById('salidaEnvio');

            someCheckbox.addEventListener('change', e => {
              if(e.target.checked === true) {
                // alert("Checkbox is checked - boolean value: ", e.target.checked)
                output.innerHTML = "Sí.";
              }
            if(e.target.checked === false) {
                // alert("Checkbox is not checked - boolean value: ", e.target.checked)
                output.innerHTML = "No.";
              }
            });
