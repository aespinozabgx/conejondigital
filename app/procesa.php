<?php
    ...
    if (isset($_POST['btnReestablecePassword']))
    {         
         
        
        $idCliente = $_POST['emailReestablece'];

        // Validar el formato del correo electrónico
        if (filter_var($idCliente, FILTER_VALIDATE_EMAIL)) 
        {
            // Limpiar el correo electrónico
            $idCliente = htmlspecialchars($idCliente, ENT_QUOTES, 'UTF-8');

            // Continuar con el procesamiento
        } else {
            // No es un correo electrónico válido, manejar el error
            die("No es un correo electrónico válido, manejar el error");
            exit;
        }
        ...
        if ($datosUsuario != false)
        {
            ...
            {
                // Cuenta activa, login
                header('Location: ../login.php?msg=cuentaInactiva&correo=' . $idCliente);
                exit();
            }
        }        
    ...
    { 
        ...
        //echo "btnReenvioActivacionCuenta";
        
        $emailActivacion = htmlentities($_POST['emailReactivacion']);
        
        $nombreRecipiente = $emailActivacion; 
                            ...
                            <tr>
                                ...
                                <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                                    ...
                                    <td>
                                        ...
                                        <p>Tendrás acceso a lso eventos del conejón, promociones especiales, a la tienda Oficial del Conejón Digital y mucho más.</p>';
        ...
        {
            header("Location: ../registro.php?email=". $emailActivacion ."&msg=errorRegistroVIP");
            exit();
        } 
    }
    ...
    if (isset($_POST['btnCrearPedido'])) 
    {
        ...
        try 
        {
            ...
            $sqlNuevoPedido = "INSERT INTO pedidos (idPedido, idTienda, idCliente, fechaPedido, idTipoEnvio, direccionEnvio, idEstatusPedido, idMetodoDePago, precioEnvio, subtotal, descuentos,  total, isActive)
            VALUES ('$idPedido', '$idTienda', '$idCliente', '$fechaPedido', '$tipoEnvio', '$direccionEnvio', '$idEstatusPedido', '$idMetodoDePago', '$precioEnvio', '$subtotal', '$descuentos', '$total', '$isActive')";
    ...
    if (isset($_POST['btnLogin28'])) 
    {
        ...
        if (isset($_POST['form_email'], $_POST['form_password'])) 
        {
            ...
            $inputPassword = isset($_POST['form_password']) ? trim($_POST['form_password']) : null;
            ...
            if (!empty($inputPassword)) {
                ...
                if (!empty($form_email)) {
                    ...
                    // Utilizamos una sentencia preparada para evitar ataques de inyección SQL
                    $stmt = mysqli_prepare($conn, "SELECT * FROM `usuarios` WHERE email = ?");
                    
                    // Vinculamos el parámetro
                    mysqli_stmt_bind_param($stmt, "s", $form_email);
                    ...
                    if (mysqli_num_rows($query) > 0) 
                    {
                        ...
                        //$verifico_password = password_verify($inputPassword, $usuario_db_pass);
                ...
                // Redireccionamos con el mensaje de error
                header('Location: ../login.php?msg=' . $error_message . "&email=" . $form_email);
                exit();
            } else {
                // Manejar el caso de contraseña vacía, por ejemplo, mostrar un mensaje de error al usuario.
                $error_message = "errorContraseñaVacia";
                header('Location: ../login.php?msg=' . $error_message . "&email=" . $form_email);
                exit();
            }
        } else {
            $error_message = "errorDatosFaltantes33";
            header('Location: ../login.php?msg=' . $error_message . "&email=" . $form_email);
            exit();
        }
    }
